...
class PrivacyTileService : TileService() {
    ...
    override fun onClick() {
        ...
        val nextArmed = !prefs.isProtectionArmedSync()
        prefs.setProtectionArmed(nextArmed)

        if (nextArmed) {
            PrivacyOverlayService.armSentinel(this)
        } else {
            PrivacyOverlayService.disarmAll(this)
        }
    ...
    private fun updateTileState() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            tile.subtitle = when {
                isBlackout -> "Panic Active"
                isArmed -> "Shield Armed"
                else -> "Shield Off"
            }
        }
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                try {
                    TileService.requestListeningState(
                        ...
                        ComponentName(context.applicationContext, PrivacyTileService::class.java)
                    )
                } catch (_: Throwable) {}
            }
        }
